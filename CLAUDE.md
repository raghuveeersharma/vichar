...
└── src/
    ...
    │   └── AuthContext.jsx           # AuthProvider: session state (mirrored to localStorage), signup/login/logout, global 401 interceptor
    ├── hooks/
    │   ├── useCachedQuery.js         # cache-first read of one resource: render the stored copy, then revalidate
    │   ├── useOnline.js              # navigator.onLine + the online/offline events
    │   └── useSpeechToText.js        # Web Speech API lifecycle: final transcripts out, interim text for preview
    ├── pages/
    ...
    ├── components/
    │   ├── RichTextEditor.jsx        # TipTap editor + toolbar + the two AI buttons + the dictation mic
    ...
    │   ├── ConfirmDialog.jsx         # native <dialog> yes/no prompt; guards every delete
    ...
    │   ├── OfflineNotice.jsx         # "nothing cached for this view" state
    │   ├── SyncStatus.jsx            # connectivity bar; owns the outbox flush
    ...
    └── libs/
        ...
        ├── idb.js                    # the only IndexedDB plumbing: one db, `entries` + `outbox` stores, fail-soft
        ├── cache.js                  # cached responses keyed by owner + resource, and the note write-through
        ├── outbox.js                 # storage for writes made offline (queue only, no requests)
        ├── notes.js                  # every note mutation: server + cache, or the queue when offline
...
Routes are declared in `App.jsx` as two guarded groups: `/login` and `/signup` behind `GuestRoute`, and `/`, `/create`, `/note/:id`, `/folder/:folderId`, `/settings` behind `ProtectedRoute`. There is no data-fetching library — reads go through [useCachedQuery.js](front/src/hooks/useCachedQuery.js) and writes through [libs/notes.js](front/src/libs/notes.js), both of which call the shared `api` instance. `AuthContext` is the only global state. `NoteCard` receives `setNotes` from its page so it can splice a deleted note out of the visible list; that prop-drilled setter is the only cross-component state channel.
...
**Session handling on the client is centralised in two places.** `AuthProvider` exposes `checking`, which is true until the boot `GET /auth/me` settles; both route guards must render a spinner while it is true, or a signed-in user gets bounced to `/login` on every refresh. It also installs an axios response interceptor that clears the user on any non-`/auth/` 401, which is what makes an expired cookie redirect to login on its own — so pages should stay silent on 401 rather than showing a "failed to fetch" toast. Note the asymmetry the offline cache depends on: an expired cookie clears the session but *not* the cached notes (entries are owner-scoped, so a re-login finds them warm and another account cannot read them), while an explicit logout wipes both.
...
- **No API response is ever cached *by the worker*.** Note bodies can be encrypted and every response is scoped to the signed-in user, so a copy in the Cache API would outlive the cookie that authorised it. The precache covers built assets only. Offline reads live in app state instead — see the cache-first section above — so a `runtimeCaching` rule for anything under `VITE_SERVER_URL` is not just unnecessary, it reintroduces the leak that design avoids.
...
**Reads are cache-first, and the cache is app state — not the service worker.** [useCachedQuery.js](front/src/hooks/useCachedQuery.js) reads a stored copy from IndexedDB, renders it, and only then decides whether the network is worth asking; inside a 30-second stale window (60 for folders) it is not, which is what stops ordinary navigation from re-requesting lists the user just looked at. The same mechanism is what makes the app work offline. Rules that hold it together:

- **No API response goes into the Cache API, ever.** A cached *response* is replayed to whoever holds the tab next, outliving the cookie that authorised it. An entry in [libs/cache.js](front/src/libs/cache.js) is instead stamped with the owner it was fetched for, only ever read back for that same owner, and destroyed on logout. Adding a `runtimeCaching` rule for `VITE_SERVER_URL` to [vite.config.js](front/vite.config.js) would reintroduce exactly the problem this design avoids.
- **An encrypted note's body is never written to disk.** The plaintext only exists because the server opened it for that one response, and the client has no key to seal it again — so `forStorage` drops it and sets `contentCached: false`. `NoteCard` renders that as locked and `NoteDetailPage` refuses to open the editor, for the same reason `getNoteById` answers 500 rather than sending a placeholder into the edit form: a save over a body we do not have destroys the note. `restoreWithheldContent` is the counterpart — it puts back only content the caller already held in memory, so a cache re-read cannot downgrade a page that has the real thing.
- **Cache keys are the API's own vocabulary.** `notes:<folder>` mirrors `?folder=`, including the literal `unfiled`, so a route param maps to a request and a cache key untranslated. `notes:all` is every note, matching the rule that folders narrow the home listing rather than partitioning it — which is why `listAccepts` puts a new note into `notes:all` unconditionally and into a folder list only when the folder matches.
- **Mutations write through, they do not invalidate.** Dropping the cached lists only works online; a note created with no connection has nothing to refetch from, so `syncNoteCaches` patches every affected list instead. Folder note counts are the deliberate exception — derived data, so that entry is dropped and recomputed server-side.
- **`updatedAt` travels with the data.** `writeEntry` takes the stamp rather than setting it, so re-persisting a copy that was read from cache — or patched optimistically — keeps its real age. Stamping on write would silently extend the stale window every time anything touched the entry.
- **The session is mirrored to localStorage.** The boot `GET /auth/me` is a network call, so offline it fails; treating that as "signed out" would send the user to `/login`, which also needs the network, making the PWA unreachable offline no matter how much was cached. `AuthProvider` therefore seeds `user` from the mirror and only clears it when a request comes back with an actual response (a 401). The mirror holds the same `toPublicJSON` object every page already reads and no token — the httpOnly cookie is still the only thing that authorises anything.

**Note writes survive being offline; folder writes do not.** [libs/notes.js](front/src/libs/notes.js) is the only place a note is created, edited or deleted. With a connection it hits the API and patches the cache; without one it patches the cache and appends to the [outbox](front/src/libs/outbox.js), which [SyncStatus.jsx](front/src/components/SyncStatus.jsx) replays on reconnect. What to preserve:

- **Encrypted notes are refused offline, not queued.** Queueing a create or an edit would park the plaintext in IndexedDB until a connection returned, which is the one outcome encryption exists to prevent. The refusal carries `offlineRefusal` and no `response`, so the pages' status branches fall through to its message instead of inventing a server error. A *delete* is fine to queue — no body, no key.
- **Queued writes collapse.** A create then an edit is one create with the newer body; two edits are one update, because the server would only keep the last anyway; deleting a note that never reached the server just drops its queued create. Appending blindly would replay work the user has already superseded.
- **A `local:` id is never sent in a URL.** It is a note the server has never issued an id for, so an edit folds into its pending create *even when online* — that covers the window between reconnecting and the flush completing — and `NoteDetailPage` reads it `cacheOnly`, because a request could only 404.
- **Replay distinguishes retryable from final.** A transport failure, 401, 429 or 5xx says nothing about the item, so the flush stops and keeps the queue intact. A 4xx will say the same thing forever, so that item is dropped and reported — otherwise one permanently rejected write wedges everything queued behind it. A `DELETE` that 404s is counted as success: the note is gone, which is what was asked.
- **The flush is not triggered by the pending count.** `SyncStatus` flushes on mount and on the offline→online transition only. Keying it on the queue length would make a flush that stopped on a retryable failure immediately re-run itself, which is a request loop rather than a retry — that case is the explicit retry button.
- **IndexedDB is treated as optional.** [libs/idb.js](front/src/libs/idb.js) resolves a fallback instead of rejecting, because private windows, storage eviction and locked-down iOS all make it unavailable. A cache that cannot open degrades to no cache, never to a broken app — so no call site handles a storage error.

**Dictation is browser-native, and only finalised speech reaches the document.** [useSpeechToText.js](front/src/hooks/useSpeechToText.js) wraps `window.SpeechRecognition ?? window.webkitSpeechRecognition` in continuous + interim mode and hands the editor nothing but completed phrases; there is no backend route, no dependency and no key, which is the whole reason this feature is free where `/api/ai` is metered. Six rules hold it together:

- **Interim results are preview-only, and that is the point of the hook.** The recogniser rewrites in-progress text as it hears more of a sentence, so anything inserted from it would be words the user never said. Finals go to the caller's `onFinalResult`; interim text is exposed as `interimTranscript` for a muted inline preview and never touches the document. Do not "helpfully" insert it as a placeholder — there is no way to reliably take it back once TipTap has it in the undo stack.
- **`onresult` must start at `event.resultIndex`.** In continuous mode `event.results` keeps growing, so re-reading it from 0 re-commits every sentence already inserted.
- **`onend` is the single place the listening state resets.** Chrome ends recognition on its own after a stretch of silence even with `continuous = true`, so resetting only in `stop()` leaves the button stuck mid-dictation with no live recogniser behind it.
- **Every handler checks it is still the current instance.** A stopped recogniser emits trailing events after a replacement exists, and an unguarded `onend` switches the UI off while the new instance is genuinely listening. Same reason unmount detaches the handlers *before* calling `abort()`.
- **Errors are reported through a callback, not read off state.** The codes repeat — two `no-speech` events set the same value — so an effect watching `error` fires once and then goes quiet. `no-speech` and `aborted` are deliberately silent in the editor: the first fires on any ordinary pause, the second is raised by our own `stop()` and teardown.
- **Recognition is not local, so it is refused offline** (`useOnline`) rather than started and left to hang against the vendor's speech service; it also needs a secure context, which covers `localhost` and the HTTPS deployment but nothing else. An unsupported browser (Firefox) keeps the button visible and toasts, so the toolbar does not silently change shape between browsers. `runAI` stops dictation before it sends, because its reply lands as a whole-document `setContent` that would discard anything spoken meanwhile — and the mic is disabled for the duration, so a still-listening recogniser would have no off switch.

**Frontend conventions.** Rich text via `@tiptap/react` v3 — note that `useEditor` does **not** re-render on every transaction in v3, so toolbar active states must come from `useEditorState`, and the editor is uncontrolled: `value` is only pushed in via `setContent` when it differs from `editor.getHTML()`, or the caret jumps on every keystroke. Editor content is styled with the `prose` classes from `@tailwindcss/typography`. Routing via `react-router` v7 (`Routes` in [App.jsx](front/src/App.jsx), `BrowserRouter` in [main.jsx](front/src/main.jsx)); `vercel.json` rewrites all paths to `/` for SPA deep links. Toasts via `react-hot-toast` (single `<Toaster />` at the root). Destructive actions confirm through [ConfirmDialog.jsx](front/src/components/ConfirmDialog.jsx), never `window.confirm` — the native prompt blocks the tab, ignores the theme, and renders as a bare system sheet in the installed PWA. It is a native `<dialog>` for the same reason `FolderNameDialog` is, and it awaits `onConfirm` so a slow delete spins inside the dialog rather than behind it. Two things it demands of callers: inside a card that is itself a `<Link>` (`NoteCard`) the dialog must be a **sibling** of the anchor, or clicking Cancel bubbles out and navigates; and a check that would refuse the action outright — `requireConnection` on the folder pages — belongs *before* the prompt, so the user is never asked to confirm something that was never going to run. Icons from `lucide-react`. Styling is Tailwind + daisyUI locked to the `forest` theme — use daisyUI semantic classes (`text-primary`, `base-content`, `bg-primary/10`) rather than raw colors.
