...
const NoteCard = ({ note, setNotes, showFolder = true }) => {
  ...
  const handelDelete = async () => {
    try {
      const { queued } = await deleteNote(user?._id, note._id);
  ...
  return (
    // The dialog is a sibling of the card, never a child of it: the whole card is
    // an anchor, and a click inside a dialog nested in it would bubble back out
    // and navigate. It is out of flow whether open or not, so it takes no cell in
    // the grid this card sits in.
    <>
      ...
      >
        <div className="card-body">
          {/* Reads as a breadcrumb above the title. Plain text, not a link: the
              whole card is already an anchor, and nesting one inside it is invalid
              HTML — browsers close the outer <a> early and the layout falls apart.
          ...
          <div className="card-actions mt-auto items-center justify-between pt-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                onClick={(e) => {
                  // The tile is a Link, so opening the prompt has to stop the
                  // navigation the click would otherwise trigger.
                  e.preventDefault();
                  setConfirming(true);
                }}
              />
      ...
      <ConfirmDialog
        open={confirming}
        title="Delete note"
        message={`"${note.title}" will be deleted. This cannot be undone.`}
